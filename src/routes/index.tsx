...
function Index() {
  ...
  // Doğrulama henüz yapılmadıysa gösterilecek ekran
  if (!isVerified) {
    return (
      <div className="hero-surface flex min-h-screen items-center justify-center px-6">
        <CustomCursor />
        <div className="reveal flex flex-col items-center text-center max-w-md p-8 rounded-lg border border-border bg-card/60 backdrop-blur-md">
          <img
            src={logo.url}
            alt="ForeZone Co. monogram"
            className="animate-float w-16 h-16 rounded-sm mb-6"
          />
          <span className="text-[11px] tracking-[0.3em] uppercase text-muted-foreground mb-2">
            Security Check
          </span>
          <h2 className="text-2xl font-display mb-3">Verify you are human</h2>
  ...
  // Doğrulama tamamlandıktan sonra sitenin kendisi açılır
  return (
